...
namespace OopProjectGui
{
    class Flight
    {
        ...
        public int findPassenger(int cId)
        {
            for (int x = 0; x < maxSeats; x++)
            {
                if (passengers[x].getId() == cId)
                    return x;
            }
            return -1;
        }

        public bool removePassenger(int cId)
        {
            int loc = findPassenger(cId);
        ...
        public string getPassengerList()
        {
            if (numPassengers < 1)
            {
                return "\nThere are no Passengers on this flight yet.";
            }
